  ...
  def __getitem__(self, index):
    ...
    if self.split == 'train':
      if not self.opt.not_rand_crop:
        s = s * np.random.choice(np.arange(0.3, 1.2, 0.1))   ## init comment
        s = s
    ...
    mask1d = np.zeros((output_res, output_res), dtype=np.uint8)
    ...
    for k in range(num_objs):
      ...
      if   not self.split == 'train':
          ...
          pts = np.array(ann['keypoints'][0:num_joints*3], np.float32).reshape(num_joints, 3)
      if flipped:
        bbox[[0, 2]] = width - bbox[[2, 0]] - 1
        pts[:, 0] = width - pts[:, 0] - 1
        for e in self.flip_idx:
    ...
    if rot != 0:
      ...
      reg_mask *= 0
      kps_mask *= 0
    if self.opt.simi and not self.opt.ss:
        ret = {'input': inp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh,
               'landmarks': kps, 'hps_mask': kps_mask, 'wight_mask': wight_mask,
               'bboxf':BBox_f,'bboxm':BBox_m,'mask1d':mask1d}
    else:
...
def Multiposebatch(batch):
  ...
  for key in batch[0]:
    #if key in ['hm', 'input']:
    if key in ['hm', 'input','inp1','inp2','inp3']:
