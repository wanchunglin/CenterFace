...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet | centerface_hp')
    ...
    self.parser.add_argument('--save_dir_name', type=str, 
                             default='res_18_512_2cls_AIZOO_0rot_0.8crop_maxobjs32_consis(w100,normed,ratioed_mask0.25)',#5e-4 _simi3(h88-8_w66-8_channel256_margin0.1_margint0.1_w1)
                                 help='model saved file name.')
    
    
    self.parser.add_argument('--iterdet', type=int, default=0,#5e-4
                                 help='use iterdet or not.')
    self.parser.add_argument('--attention', type=int, default=0,#5e-4
                             help='1/2 for attention(CBAM module) in basicblock or at layer before head layers.')
    self.parser.add_argument('--concat', type=int, default=0,#5e-4
                             help='use concat at attention or not.')
    
    
    self.parser.add_argument('--simi', type=float, default=0,
                             help='Use triples or not(triples loss)')
    self.parser.add_argument('--simi_weight', type=float, default=1,
                             help='triples weight')
    self.parser.add_argument('--consis', type=float, default=0,
                             help='Use self supervised consistcy loss (flip loss)')
    self.parser.add_argument('--consis_weight', type=float, default=100,
                             help='consis weight')
    
    self.parser.add_argument('--ss', type=int, default=0,#5e-4
                                 help='use self-supervise type 1 for consistency, 2 for rotation prediction.')
    self.parser.add_argument('--img2_type', type=int, default=2,#5e-4
                                 help='1 for gray 2 for random crop.')
    self.parser.add_argument('--ss_weight', type=float, default=1,#5e-4
                                 help='self-supervise weight.')
    

    self.parser.add_argument('--lr', type=float, default=1.25e-4,#5e-4,
                             help='learning rate for batch size 32.')
    self.parser.add_argument('--lr_step', type=str, default='30,80',#90,120
    ...
    self.parser.add_argument('--nms', default = False,action='store_true',
    ...
    self.parser.add_argument('--no_color_aug', action='store_true',
                             help='not use the color augmenation '
                                  'from CornerNet')
    self.parser.add_argument('--random_rotate', type = float, default=0,
                             help='probability of applying random_rotate.')
    self.parser.add_argument('--random_crop_rate', type = float, default=0.8,
  ...
  def parse(self, args=''):
    ...
    opt.save_dir = os.path.join(opt.exp_dir, opt.save_dir_name)
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      #if opt.simi:
      #    opt.heads.update({'simi': opt.num_classes})
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
